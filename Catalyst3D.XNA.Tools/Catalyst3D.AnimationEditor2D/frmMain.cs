	...
	public partial class frmMain : Form
	{
		...
		private void OnObjectSelected(VisualObject obj)
		{
			if(obj == null)
			{
				...
				col.AddRange(conRenderer.RendererWindow.VisualObjects);
				col.AddRange(conRenderer.Paths.ToArray());

				foreach(VisualObject vo in col)
					vo.IsSelected = false;

				Globals.CurrentSelectedObject = null;
		...
		{
			VisualObject vo = Globals.CurrentSelectedObject;

			if (vo is LedgeBuilder)
			{
				var ledge = vo as LedgeBuilder;

				var objects = (from s in Globals.RenderWindow.SceneManager.CurrentScreen.VisualObjects
		...
		{
			if (Globals.CurrentSelectedObject == null)
				return;

			#region Sprites

			if (Globals.CurrentSelectedObject is EditorSprite)
			{
				EditorSprite original = Globals.CurrentSelectedObject as EditorSprite;
			...
			if (Globals.CurrentSelectedObject is ParticleEmitter)
			{
				EditorEmitter original = Globals.CurrentSelectedObject as EditorEmitter;

				if (original != null)
				{
					EditorEmitter emitter = new EditorEmitter(Globals.Game)
																		{
																			Name = original.Name,
																			AssetName = original.AssetName,
			...
			if (Globals.CurrentSelectedObject is VisualObjectGroup)
			{
				EditorGroup original = Globals.CurrentSelectedObject as EditorGroup;

				if (original != null)
				{
					EditorGroup group = new EditorGroup(Globals.Game)
																{
																	AssetName = original.AssetName,
																	DrawOrder = original.DrawOrder,
																	UpdateOrder = original.UpdateOrder,
			...
			if (Globals.CurrentSelectedObject is EditorActor)
			{
				EditorActor proxy = Globals.CurrentSelectedObject as EditorActor;

				if (proxy != null)
				{
					EditorActor actor = new EditorActor(Globals.Game, string.Empty, proxy.AssetName);
			...
			if (Globals.CurrentSelectedObject is EditorButton)
			{
				Catalyst3D.XNA.Engine.EntityClasses.UI.Button original =
					Globals.CurrentSelectedObject as Catalyst3D.XNA.Engine.EntityClasses.UI.Button;

				EditorButton button = new EditorButton(Globals.Game);
				if (original != null)
				{
			...
			if (Globals.CurrentSelectedObject is LedgeBuilder)
			{
				var ledge = Globals.CurrentSelectedObject as LedgeBuilder;

				if (ledge != null)
				{
					var newLedge = new LedgeBuilder(Globals.Game)
					               	{
					...
					foreach (var vo in conRenderer.RendererWindow.VisualObjects)
					{
						#region Sprites
						...
						if (vo is EditorGroup)
						{
							...
							{
								EditorGroup gp = new EditorGroup(Globals.Game);
						...
						if (vo is EditorActor)
						{
							...
							if (proxy.AttachedPathingNode == ledge)
							{
								...
								actor.GameScreen = conRenderer.RendererWindow;
					...
					conRenderer.Paths.Add(newLedge);
		...
		private void LoadProject(bool showResult)
		{
			...
			{
				conRenderer.Paths.Clear();

				if (conRenderer.RendererWindow.VisualObjects.Count > 0)
					conRenderer.RendererWindow.VisualObjects.Clear();
		...
		private void LoadProject(bool result, string filename)
		{
			try
			{
				...
				{
					LedgeBuilder p = new LedgeBuilder(Globals.Game);
					p.Role = l.Role;
					p.Name = l.Name;
				...
				foreach (VisualObject obj in proj.SceneObjects)
				{
					...
					if (obj is Label)
					{
						...
						EditorLabel label = new EditorLabel(Globals.Game, obj.AssetName);
					...
					if (obj is SpriteBox)
					{
						...
						EditorSpriteBox sprite = new EditorSpriteBox(Globals.Game);
						sprite.Name = original.Name;
				...
				{
					var vo = (from s in conRenderer.SceneManager.CurrentScreen.VisualObjects
		...
		private void SaveProject(bool showResult)
		{
			...
			foreach (LedgeBuilder original in conRenderer.Paths)
			{
			...
			foreach (VisualObject t in conRenderer.RendererWindow.VisualObjects)
			{
				#region Buttons
		...
		private void pgGrid_SelectedGridItemChanged(object sender, SelectedGridItemChangedEventArgs e)
		{
			if (e.NewSelection.Value != null && e.OldSelection != null)
			{
				...
				if (v != null)
					Globals.CurrentSelectedObject = v;
			}
		}

		private void conContainer1_SizeChanged(object sender, EventArgs e)
		{
		...
		{
			if (conRenderer.PathingDisplay.CurrentPath != null || !Globals.IsProjectLoaded)
				return;
		...
		private void newProjectToolStripMenuItem_Click(object sender, EventArgs e)
		{
			...
			if (dia.ShowDialog() == DialogResult.OK)
			{
				...
				conRenderer.RendererWindow.VisualObjects = new List<VisualObject>();
		...
		{
			frmGroupObjects group = new frmGroupObjects(conRenderer.RendererWindow.VisualObjects);
		...
		private void btnDuplicateSpecial_Click(object sender, EventArgs e)
		{
			...
			frmDuplicateSpecial frm = new frmDuplicateSpecial(conRenderer.RendererWindow.VisualObjects);
			frm.Show();
		}
		...
		private void tsSavePath_Click(object sender, EventArgs e)
		{
			...
			Globals.ObjectSelected.Invoke(conRenderer.PathingDisplay.CurrentPath);
			...
			if (Globals.IsProjectLoaded)
			{
				...
				if (dia.ShowDialog() == DialogResult.OK)
				{
					try
					{
						...
						EditorModel model = new EditorModel(Globals.Game, Globals.Camera);
						model.Initialize();
						...
						if (string.IsNullOrEmpty(result))
						{
							...
							model.Content = content.Load<Microsoft.Xna.Framework.Graphics.Model>(Path.GetFileNameWithoutExtension(dia.FileName));
		...
		private void undoToolStripMenuItem_Click(object sender, EventArgs e)
		{
			if (Globals.ObjectUndo != null)
			{
				...
				Globals.ObjectUndo.Invoke(Globals.CurrentSelectedObject);
			}
		}

		private void tsAddLabel_Click(object sender, EventArgs e)
		{
			frmAddLabel frm = new frmAddLabel();
			frm.Show();
		}
		...
		private void tsAttachNodes_Click(object sender, EventArgs e)
		{
			...
			frm.SceneObjects = conRenderer.RendererWindow.VisualObjects;
			frm.PathingNodes = conRenderer.Paths;
			frm.Show();
		}
			...
			if (MessageBox.Show(@"Are you sure you want to export only visible scene objects to a new .c2d file?", @"Export Confirmation", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
			{
				...
				if (dia.ShowDialog() == DialogResult.OK)
				{
					...
					for (int i = 0; i < conSpriteContainer.pnContainer.Controls.Count; i++)
					{
						...
						{
							// Loop thru our scene objects
							foreach (VisualObject o in conRenderer.RendererWindow.VisualObjects)
							{
								if (o.Visible)
								{
		...
		{
			EditorSpriteBox rec = new EditorSpriteBox(Globals.Game);
			rec.Name = "Rectangle";
			rec.Initialize();
		...
		private void tsFollow_Click(object sender, EventArgs e)
		{
			...
			{
				conRenderer.FollowingObject = Globals.CurrentSelectedObject;
			}
		}
